...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    ...
    val questions = postings.filter(_.postingType == 1).map(q => (q.id,q))          //filter questions and put them into a tuple (id,question)
    val answers = postings.filter(_.postingType == 2).map(a => (a.parentId.get,a))  //filter answers and put them into a tuple (parentId,question)
    
    questions.join(answers) //pair together each question with every of its answers
             .groupByKey()  //group the (QID,(question,answer)) tuples by QID
  }
