...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    //Group by closest mean
    val grouped = points.groupBy(findClosest(_, means))
    
    //Return a genMap with all initial means as keys, even if no point is close to them
    means.map( mean => (mean,grouped.getOrElse(mean, GenSeq() )) ).toMap
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    ...
    while(i<oldMeans.length){
      ...
      //if any mean has moved by more than eta => return false
      if(oldMeans(i).squareDistance(newMeans(i))>eta)
        return false
        
      i += 1
    }
    //no mean moved a distance more than eta
    return true
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    //return the new means if converged, call kmeans with the new means
    if (converged(eta)(means,newMeans)) newMeans else kMeans(points, newMeans, eta) // your implementation need to be tail recursive
  }
}
